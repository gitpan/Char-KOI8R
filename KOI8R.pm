...
######################################################################
#
# KOI8R - Source code filter to KOI8-R script
#
...
my $function_ord;         # ord()   to ord()   or KOI8R::ord()
my $function_ord_;        # ord     to ord     or KOI8R::ord_
my $function_reverse;     # reverse to reverse or KOI8R::reverse
...
sub KOI8R::escape_script {
    ...
    if (m/^ use Ekoi8r(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Ekoi8r %s;\n%s", $Ekoi8r::VERSION, $use_re_eval); # require run-time routines version

        # use KOI8R version qw(ord reverse);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        if (s/^ \s* use \s+ KOI8R \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list !~ m/\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'KOI8R::ord'     if m/\A ord \z/oxms;
                    $function_ord_    = 'KOI8R::ord_'    if m/\A ord \z/oxms;
                    $function_reverse = 'KOI8R::reverse' if m/\A reverse \z/oxms;
                }
            }
        }
    }
...
# functions of package Ekoi8r
    elsif (m{\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  }oxgc) { $slash = 'm//'; return $1;                    }
    elsif (m{\G \b bytes::substr \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (m{\G \b chop \b          (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Ekoi8r::chop';         }
    elsif (m{\G \b bytes::index \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'index';               }
    elsif (m{\G \b KOI8R::index \b   (?! \s* => )                }oxgc) { $slash = 'm//'; return 'KOI8R::index';         }
    elsif (m{\G \b index \b         (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Ekoi8r::index';        }
    elsif (m{\G \b bytes::rindex \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (m{\G \b KOI8R::rindex \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'KOI8R::rindex';        }
    elsif (m{\G \b rindex \b        (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Ekoi8r::rindex';       }
    elsif (m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ekoi8r::lc';           }
    elsif (m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ekoi8r::lcfirst';      }
    elsif (m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ekoi8r::uc';           }
    elsif (m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ekoi8r::ucfirst';      }
    elsif (m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ekoi8r::fc';           }
    ...
    elsif (m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ekoi8r::chr';               }
    ...
    elsif (m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ekoi8r::glob';              }
    elsif (m{\G \b lc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ekoi8r::lc_';               }
    elsif (m{\G \b lcfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ekoi8r::lcfirst_';          }
    elsif (m{\G \b uc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ekoi8r::uc_';               }
    elsif (m{\G \b ucfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ekoi8r::ucfirst_';          }
    elsif (m{\G \b fc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ekoi8r::fc_';               }
...
# functions of package Ekoi8r
        ...
        elsif ($string =~ m{\G \b chop \b                                      }oxgc) { $e_string .= 'Ekoi8r::chop';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::index \b                              }oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b KOI8R::index \b                               }oxgc) { $e_string .= 'KOI8R::index';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b index \b                                     }oxgc) { $e_string .= 'Ekoi8r::index';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::rindex \b                             }oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ m{\G \b KOI8R::rindex \b                              }oxgc) { $e_string .= 'KOI8R::rindex';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b rindex \b                                    }oxgc) { $e_string .= 'Ekoi8r::rindex';  $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ekoi8r::lc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ekoi8r::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ekoi8r::uc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ekoi8r::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ekoi8r::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ekoi8r::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ekoi8r::glob';              $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc \b                                              }oxgc) { $e_string .= 'Ekoi8r::lc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst \b                                         }oxgc) { $e_string .= 'Ekoi8r::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc \b                                              }oxgc) { $e_string .= 'Ekoi8r::uc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst \b                                         }oxgc) { $e_string .= 'Ekoi8r::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc \b                                              }oxgc) { $e_string .= 'Ekoi8r::fc_';               $slash = 'm//'; }
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ekoi8r::uc($char[$i]) ne Ekoi8r::fc($char[$i]))) {
...
KOI8R - Source code filter to escape KOI8-R

=head1 SYNOPSIS

  use KOI8R;
  use KOI8R version;         --- require version
...
=head1 Calling 'Ekoi8r::ignorecase()' (KOI8R.pm provides)

KOI8R.pm applies calling 'Ekoi8r::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ Of \N, \p, \P and \X (KOI8R.pm provides)

KOI8R.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (KOI8R.pm and Ekoi8r.pm provide)
...
=head1 Un-Escaping bytes::* Functions (KOI8R.pm provides)

KOI8R.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Ekoi8r.pm provides)

Ekoi8r.pm does "BEGIN { unshift @INC, '/Perl/site/lib/KOI8R' }" at head.
Store the standard module modified for KOI8R software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/KOI8R and change
'use utf8;' to 'use KOI8R;' in its. You need help yourself for now.
...
Contrasting is Encode module and describing "use KOI8R;" on this software,
