...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ekoi8r::uc($char[$i]) ne Ekoi8r::lc($char[$i]))) {
            $char[$i] = '[' . Ekoi8r::uc($char[$i]) . Ekoi8r::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-KOI8R/KOI8R/JA.pm
