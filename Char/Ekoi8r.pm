...
######################################################################
#
# Char::Ekoi8r - Run-time routines for Char/KOI8R.pm
#
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Char::Ekoi8r::index($name, '::') == -1) && (Char::Ekoi8r::index($name, "'") == -1)) {
            my $pkg;
...
sub import {
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        ...
        for (@ARGV) {
            ...
            # has space
            if (/\A (?:$q_char)*? [ ] /oxms) {
                if (my @glob = Char::Ekoi8r::glob(qq{"$_"})) {
                    push @argv, @glob;
                }
                else {
                    push @argv, $_;
...
#
sub unimport {}
sub Char::Ekoi8r::split(;$$$);
sub Char::Ekoi8r::tr($$$$;$);
sub Char::Ekoi8r::chop(@);
sub Char::Ekoi8r::index($$;$);
sub Char::Ekoi8r::rindex($$;$);
sub Char::Ekoi8r::lcfirst(@);
sub Char::Ekoi8r::lcfirst_();
sub Char::Ekoi8r::lc(@);
sub Char::Ekoi8r::lc_();
sub Char::Ekoi8r::ucfirst(@);
sub Char::Ekoi8r::ucfirst_();
sub Char::Ekoi8r::uc(@);
sub Char::Ekoi8r::uc_();
sub Char::Ekoi8r::fc(@);
sub Char::Ekoi8r::fc_();
sub Char::Ekoi8r::ignorecase;
sub Char::Ekoi8r::classic_character_class;
sub Char::Ekoi8r::capture;
sub Char::Ekoi8r::chr(;$);
sub Char::Ekoi8r::chr_();
sub Char::Ekoi8r::glob($);
sub Char::Ekoi8r::glob_();

sub Char::KOI8R::ord(;$);
sub Char::KOI8R::ord_();
sub Char::KOI8R::reverse(@);
sub Char::KOI8R::getc(;*@);
sub Char::KOI8R::length(;$);
sub Char::KOI8R::substr($$;$$);
sub Char::KOI8R::index($$;$);
sub Char::KOI8R::rindex($$;$);
sub Char::KOI8R::escape(;$);
...
${Char::Ekoi8r::dot}         = qr{(?:[^\x0A])};
${Char::Ekoi8r::dot_s}       = qr{(?:[\x00-\xFF])};
${Char::Ekoi8r::eD}          = qr{(?:[^0-9])};
...
# ${Char::Ekoi8r::eS}        = qr{(?:[^\x09\x0A    \x0C\x0D\x20])};
# ${Char::Ekoi8r::eS}        = qr{(?:[^\x09\x0A\x0B\x0C\x0D\x20])};
${Char::Ekoi8r::eS}          = qr{(?:[^\s])};

${Char::Ekoi8r::eW}          = qr{(?:[^0-9A-Z_a-z])};
${Char::Ekoi8r::eH}          = qr{(?:[^\x09\x20])};
${Char::Ekoi8r::eV}          = qr{(?:[^\x0A\x0B\x0C\x0D])};
${Char::Ekoi8r::eR}          = qr{(?:\x0D\x0A|[\x0A\x0D])};
${Char::Ekoi8r::eN}          = qr{(?:[^\x0A])};
${Char::Ekoi8r::not_alnum}   = qr{(?:[^\x30-\x39\x41-\x5A\x61-\x7A])};
${Char::Ekoi8r::not_alpha}   = qr{(?:[^\x41-\x5A\x61-\x7A])};
${Char::Ekoi8r::not_ascii}   = qr{(?:[^\x00-\x7F])};
${Char::Ekoi8r::not_blank}   = qr{(?:[^\x09\x20])};
${Char::Ekoi8r::not_cntrl}   = qr{(?:[^\x00-\x1F\x7F])};
${Char::Ekoi8r::not_digit}   = qr{(?:[^\x30-\x39])};
${Char::Ekoi8r::not_graph}   = qr{(?:[^\x21-\x7F])};
${Char::Ekoi8r::not_lower}   = qr{(?:[^\x61-\x7A])};
${Char::Ekoi8r::not_lower_i} = qr{(?:[\x00-\xFF])};
${Char::Ekoi8r::not_print}   = qr{(?:[^\x20-\x7F])};
${Char::Ekoi8r::not_punct}   = qr{(?:[^\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
${Char::Ekoi8r::not_space}   = qr{(?:[^\s\x0B])};
${Char::Ekoi8r::not_upper}   = qr{(?:[^\x41-\x5A])};
${Char::Ekoi8r::not_upper_i} = qr{(?:[\x00-\xFF])};
${Char::Ekoi8r::not_word}    = qr{(?:[^\x30-\x39\x41-\x5A\x5F\x61-\x7A])};
${Char::Ekoi8r::not_xdigit}  = qr{(?:[^\x30-\x39\x41-\x46\x61-\x66])};
${Char::Ekoi8r::eb}          = qr{(?:\A(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[0-9A-Z_a-z])|(?<=[0-9A-Z_a-z])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]|\z))};
${Char::Ekoi8r::eB}          = qr{(?:(?<=[0-9A-Z_a-z])(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]))};

# avoid: Name "Char::Ekoi8r::foo" used only once: possible typo at here.
${Char::Ekoi8r::dot}         = ${Char::Ekoi8r::dot};
${Char::Ekoi8r::dot_s}       = ${Char::Ekoi8r::dot_s};
${Char::Ekoi8r::eD}          = ${Char::Ekoi8r::eD};
${Char::Ekoi8r::eS}          = ${Char::Ekoi8r::eS};
${Char::Ekoi8r::eW}          = ${Char::Ekoi8r::eW};
${Char::Ekoi8r::eH}          = ${Char::Ekoi8r::eH};
${Char::Ekoi8r::eV}          = ${Char::Ekoi8r::eV};
${Char::Ekoi8r::eR}          = ${Char::Ekoi8r::eR};
${Char::Ekoi8r::eN}          = ${Char::Ekoi8r::eN};
${Char::Ekoi8r::not_alnum}   = ${Char::Ekoi8r::not_alnum};
${Char::Ekoi8r::not_alpha}   = ${Char::Ekoi8r::not_alpha};
${Char::Ekoi8r::not_ascii}   = ${Char::Ekoi8r::not_ascii};
${Char::Ekoi8r::not_blank}   = ${Char::Ekoi8r::not_blank};
${Char::Ekoi8r::not_cntrl}   = ${Char::Ekoi8r::not_cntrl};
${Char::Ekoi8r::not_digit}   = ${Char::Ekoi8r::not_digit};
${Char::Ekoi8r::not_graph}   = ${Char::Ekoi8r::not_graph};
${Char::Ekoi8r::not_lower}   = ${Char::Ekoi8r::not_lower};
${Char::Ekoi8r::not_lower_i} = ${Char::Ekoi8r::not_lower_i};
${Char::Ekoi8r::not_print}   = ${Char::Ekoi8r::not_print};
${Char::Ekoi8r::not_punct}   = ${Char::Ekoi8r::not_punct};
${Char::Ekoi8r::not_space}   = ${Char::Ekoi8r::not_space};
${Char::Ekoi8r::not_upper}   = ${Char::Ekoi8r::not_upper};
${Char::Ekoi8r::not_upper_i} = ${Char::Ekoi8r::not_upper_i};
${Char::Ekoi8r::not_word}    = ${Char::Ekoi8r::not_word};
${Char::Ekoi8r::not_xdigit}  = ${Char::Ekoi8r::not_xdigit};
${Char::Ekoi8r::eb}          = ${Char::Ekoi8r::eb};
${Char::Ekoi8r::eB}          = ${Char::Ekoi8r::eB};
...
sub Char::Ekoi8r::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Char::Ekoi8r::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Char::Ekoi8r::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Char::Ekoi8r::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
...
sub Char::Ekoi8r::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Char::Ekoi8r::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Char::Ekoi8r::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Char::Ekoi8r::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Char::Ekoi8r::uc($char[$i]);
                my $fc = Char::Ekoi8r::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Char::Ekoi8r::not_lower_i}',
                '[:^upper:]'  => '${Char::Ekoi8r::not_upper_i}',
            ...
            $char[$i] = {
                ...
                '[:^alnum:]'  => '${Char::Ekoi8r::not_alnum}',
                '[:^alpha:]'  => '${Char::Ekoi8r::not_alpha}',
                '[:^ascii:]'  => '${Char::Ekoi8r::not_ascii}',
                '[:^blank:]'  => '${Char::Ekoi8r::not_blank}',
                '[:^cntrl:]'  => '${Char::Ekoi8r::not_cntrl}',
                '[:^digit:]'  => '${Char::Ekoi8r::not_digit}',
                '[:^graph:]'  => '${Char::Ekoi8r::not_graph}',
                '[:^lower:]'  => '${Char::Ekoi8r::not_lower}',
                '[:^print:]'  => '${Char::Ekoi8r::not_print}',
                '[:^punct:]'  => '${Char::Ekoi8r::not_punct}',
                '[:^space:]'  => '${Char::Ekoi8r::not_space}',
                '[:^upper:]'  => '${Char::Ekoi8r::not_upper}',
                '[:^word:]'   => '${Char::Ekoi8r::not_word}',
                '[:^xdigit:]' => '${Char::Ekoi8r::not_xdigit}',
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Char::Ekoi8r::uc($char);
                        my $fc = Char::Ekoi8r::fc($char);
                        if ($uc eq $fc) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Char::Ekoi8r::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Char::Ekoi8r::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Char::Ekoi8r::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Char::Ekoi8r::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
sub Char::KOI8R::getc(;*@) {
    ...
    croak 'Too many arguments for Char::KOI8R::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Char::Ekoi8r::dot_s} \z/oxms) {
...
sub Char::KOI8R::index($$;$) {
    ...
    my $index;
    if (@_ == 3) {
        $index = Char::Ekoi8r::index($_[0], $_[1], CORE::length(Char::KOI8R::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Char::Ekoi8r::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Char::KOI8R::length(CORE::substr $_[0], 0, $index);
    }
}
...
sub Char::KOI8R::rindex($$;$) {
    ...
    my $rindex;
    if (@_ == 3) {
        $rindex = Char::Ekoi8r::rindex($_[0], $_[1], CORE::length(Char::KOI8R::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Char::Ekoi8r::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Char::KOI8R::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?: Char::KOI8R::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
sub Char::KOI8R::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Char::KOI8R::escape_token();
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Ekoi8r::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ekoi8r::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Ekoi8r::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ekoi8r::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Ekoi8r::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Ekoi8r::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Char::Ekoi8r::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Ekoi8r::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Ekoi8r::glob';
    }
...
# subroutines of package Char::Ekoi8r
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Char::KOI8R::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::KOI8R::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Char::KOI8R::escape'; }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::chop';       }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::KOI8R::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::KOI8R::index';       }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::index';      }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::KOI8R::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::KOI8R::rindex';      }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::rindex';     }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::lc';         }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::lcfirst';    }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::uc';         }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::ucfirst';    }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::fc';         }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Ekoi8r::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Ekoi8r::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Ekoi8r::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Ekoi8r::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Ekoi8r::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Char::Ekoi8r::PREMATCH()]}
            ...
            $e_string .= q{Char::Ekoi8r::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Char::Ekoi8r::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Char::Ekoi8r::MATCH()};
            $slash = 'div';
        }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> @{[Char::Ekoi8r::POSTMATCH()]}
            ...
            $e_string .= q{Char::Ekoi8r::POSTMATCH()};
            $slash = 'div';
        }
...
# subroutines of package Char::Ekoi8r
        ...
        elsif ($string =~ /\G \b Char::KOI8R::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::KOI8R::eval \b                                /oxgc) { $e_string .= 'eval Char::KOI8R::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                             /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Ekoi8r::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::KOI8R::index \b                               /oxgc) { $e_string .= 'Char::KOI8R::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Ekoi8r::index';      $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';            $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::KOI8R::rindex \b                              /oxgc) { $e_string .= 'Char::KOI8R::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Ekoi8r::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8r::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8r::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8r::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8r::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8r::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8r::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Ekoi8r::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Ekoi8r::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Ekoi8r::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Ekoi8r::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Ekoi8r::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Ekoi8r::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Ekoi8r::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Ekoi8r::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Ekoi8r::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Ekoi8r::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Ekoi8r::dot_s}';
        }
        else {
            return '${Char::Ekoi8r::dot}';
        }
    }
    else {
        return Char::Ekoi8r::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Ekoi8r::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Ekoi8r::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Ekoi8r::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Ekoi8r::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Ekoi8r::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Ekoi8r::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Ekoi8r::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Ekoi8r::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Ekoi8r::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Ekoi8r::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Ekoi8r::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Ekoi8r::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Ekoi8r::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Ekoi8r::uc($char[$i]) ne Char::Ekoi8r::fc($char[$i]))) {
            if (CORE::length(Char::Ekoi8r::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Ekoi8r::uc($char[$i])       . Char::Ekoi8r::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Ekoi8r::uc($char[$i]) . '|' . Char::Ekoi8r::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8r::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Ekoi8r::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8r::ignorecase(Char::Ekoi8r::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ekoi8r::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Ekoi8r::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8r::ignorecase(Char::Ekoi8r::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ekoi8r::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Ekoi8r::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8r::ignorecase(Char::Ekoi8r::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Ekoi8r::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Char::KOI8R::re_t=%s; while($Char::KOI8R::re_t =~ %s){%s local $^W=0; local $Char::KOI8R::re_r=%s; %s$Char::KOI8R::re_t="$`$Char::KOI8R::re_r$'"; pos($Char::KOI8R::re_t)=length "$`$Char::KOI8R::re_r"; } return $Char::KOI8R::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::KOI8R::re_r=CORE::eval $Char::KOI8R::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Char::KOI8R::re_r=%s; %s"%s$Char::KOI8R::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Char::KOI8R::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::KOI8R::re_r=%s; %s%s="$`$Char::KOI8R::re_r$'"; pos(%s)=length "$`$Char::KOI8R::re_r"; $Char::KOI8R::re_n++} return %s$Char::KOI8R::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Char::KOI8R::re_r=%s; %s%s="%s$Char::KOI8R::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Char::KOI8R::re_r=%s; %s%s="%s$Char::KOI8R::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::KOI8R::re_r=CORE::eval $Char::KOI8R::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Ekoi8r::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Ekoi8r::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Ekoi8r::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=head1 SYNOPSIS
  ...
  use Char::Ekoi8r;
    ...
    Char::Ekoi8r::lc_;
    Char::Ekoi8r::lcfirst(...);
    Char::Ekoi8r::lcfirst_;
    Char::Ekoi8r::uc(...);
    Char::Ekoi8r::uc_;
    Char::Ekoi8r::ucfirst(...);
    Char::Ekoi8r::ucfirst_;
    Char::Ekoi8r::fc(...);
    Char::Ekoi8r::fc_;
    Char::Ekoi8r::ignorecase(...);
    Char::Ekoi8r::capture(...);
    Char::Ekoi8r::chr(...);
    Char::Ekoi8r::chr_;
    Char::Ekoi8r::glob(...);
  ...
  # "no Char::Ekoi8r;" not supported

=head1 ABSTRACT

This module has run-time routines for use Char::KOI8R software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in Char::KOI8R software and not
...
This Char::Ekoi8r module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Char::Ekoi8r::split(/pattern/,$string,$limit);
  @split = Char::Ekoi8r::split(/pattern/,$string);
  @split = Char::Ekoi8r::split(/pattern/);
  @split = Char::Ekoi8r::split('',$string,$limit);
  @split = Char::Ekoi8r::split('',$string);
  @split = Char::Ekoi8r::split('');
  @split = Char::Ekoi8r::split();
  @split = Char::Ekoi8r::split;
  ...
  at all, Char::Ekoi8r::split returns the original string as a single substring, If it
  ...
  @chars  = Char::Ekoi8r::split(//,  $word);
  @fields = Char::Ekoi8r::split(/:/, $line);
  @words  = Char::Ekoi8r::split(" ", $paragraph);
  @lines  = Char::Ekoi8r::split(/^/, $buffer);
  ...
  print join(":" => Char::Ekoi8r::split(/ */, "hi there"));
  ...
  Char::Ekoi8r::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Char::Ekoi8r::split(/:/, $_, 3);
  ...
  splits on whitespace just as Char::Ekoi8r::split with no arguments does. Thus,
  Char::Ekoi8r::split(" ") can be used to emulate awk's default behavior. In contrast,
  Char::Ekoi8r::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Char::Ekoi8r::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Char::Ekoi8r::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Char::Ekoi8r::split()) {
          $count{$word}++;
      }
  }

  The inverse of Char::Ekoi8r::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Char::Ekoi8r::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Char::Ekoi8r::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  print Char::Ekoi8r::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Char::Ekoi8r::chop(@list);
  $chop = Char::Ekoi8r::chop();
  $chop = Char::Ekoi8r::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Char::Ekoi8r::chop subroutine is used primary to remove the newline
  ...
  since Char::Ekoi8r::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Char::Ekoi8r::chop a literal, only a variable. If you Char::Ekoi8r::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Char::Ekoi8r::chop(@lines);

  You can Char::Ekoi8r::chop anything that is an lvalue, including an assignment:

  Char::Ekoi8r::chop($cwd = `pwd`);
  Char::Ekoi8r::chop($answer = <STDIN>);

  This is different from:

  $answer = Char::Ekoi8r::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Char::Ekoi8r::chop returns the character
  ...
  In the most general case, Char::Ekoi8r::chop can be expressed using substr:

  $last_code = Char::Ekoi8r::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Char::Ekoi8r::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Char::Ekoi8r::chop;
...
=item Index string
  ...
  $byte_pos = Char::Ekoi8r::index($string,$substr,$byte_offset);
  $byte_pos = Char::Ekoi8r::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Char::Ekoi8r::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Char::Ekoi8r::rindex($string,$substr,$byte_offset);
  $byte_pos = Char::Ekoi8r::rindex($string,$substr);

  This subroutine works just like Char::Ekoi8r::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Char::Ekoi8r::index).
  ...
  while (($byte_pos = Char::KOI8R::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Char::Ekoi8r::fc subroutine for case-insensitive comparisons via Char::KOI8R
  software.

=item Lower case first character of string

  $lcfirst = Char::Ekoi8r::lcfirst($string);
  $lcfirst = Char::Ekoi8r::lcfirst_;
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Char::Ekoi8r::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Char::Ekoi8r::ucfirst($string);
  $ucfirst = Char::Ekoi8r::ucfirst_;
  ...
  $titlecase = Char::Ekoi8r::ucfirst(substr($word,0,1)) . Char::Ekoi8r::lc(substr($word,1));
  ...
  $do_not_use = Char::Ekoi8r::ucfirst(Char::Ekoi8r::lc($word));
...
=item Fold case string
  ...
  $fc = Char::Ekoi8r::fc($string);
  $fc = Char::Ekoi8r::fc_;

  New to Char::KOI8R software, this subroutine returns the full Unicode-like casefold of
  ...
  Char::Ekoi8r::fc($a) eq Char::Ekoi8r::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Char::KOI8R software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Char::Ekoi8r::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item Make capture number

  $capturenumber = Char::Ekoi8r::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item Make character

  $chr = Char::Ekoi8r::chr($code);
  $chr = Char::Ekoi8r::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Char::Ekoi8r::chr(65) is "A" in either
  ASCII or KOI8-R, not Unicode. For the reverse of Char::Ekoi8r::chr, use Char::KOI8R::ord.

=item Filename expansion (globbing)

  @glob = Char::Ekoi8r::glob($string);
  @glob = Char::Ekoi8r::glob_;
  ...
  The Char::Ekoi8r::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Char::Ekoi8r::glob('"*e f*"');
  @spacies = Char::Ekoi8r::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Char::Ekoi8r::glob("'*${var}e f*'");
  @spacies = Char::Ekoi8r::glob(qq("*${var}e f*"));
