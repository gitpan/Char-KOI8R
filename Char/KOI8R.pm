...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Ekoi8r::uc($char[$i]) ne Char::Ekoi8r::lc($char[$i]))) {
            $char[$i] = '[' . Char::Ekoi8r::uc($char[$i]) . Char::Ekoi8r::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::KOI8R/Char::KOI8R/JA.pm
