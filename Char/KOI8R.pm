...
######################################################################
#
# Char::KOI8R - Source code filter to escape KOI8-R script
#
# http://search.cpan.org/dist/Char-Char::KOI8R/
#
...
BEGIN { CORE::require Char::Ekoi8r; }
...
my $function_ord;         # ord()   to ord()   or Char::KOI8R::ord()
my $function_ord_;        # ord     to ord     or Char::KOI8R::ord_
my $function_reverse;     # reverse to reverse or Char::KOI8R::reverse
my $function_getc;        # getc    to getc    or Char::KOI8R::getc
...
END
    ...
    print Char::KOI8R::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Ekoi8r::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::KOI8R::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Ekoi8r::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::KOI8R::escape_script {
    ...
    # read KOI8-R script
    my $fh = gensym();
    Char::Ekoi8r::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ekoi8r(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ekoi8r %s;\n", $Char::KOI8R::VERSION); # require run-time routines version

        # use Char::KOI8R version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::KOI8R \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::KOI8R::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/KOI8R.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/KOI8R.pm $version, but @{[__FILE__]} is version $Char::KOI8R::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ekoi8r.pm $version, but \\\$Char::Ekoi8r::VERSION is \$Char::Ekoi8r::VERSION" if \$Char::Ekoi8r::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::KOI8R::VERSION) {
                    die "Script $0 required Char/KOI8R.pm $version, but @{[__FILE__]} is only version $Char::KOI8R::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::KOI8R::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::KOI8R::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::KOI8R::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::KOI8R::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Ekoi8r
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::KOI8R::re_t=%s; while($Char::KOI8R::re_t =~ %s){%s local $^W=0; local $Char::KOI8R::re_r=%s; %s$Char::KOI8R::re_t="$`$Char::KOI8R::re_r$'"; pos($Char::KOI8R::re_t)=length "$`$Char::KOI8R::re_r"; } return $Char::KOI8R::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::KOI8R::re_r=eval $Char::KOI8R::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::KOI8R::re_r=%s; %s"%s$Char::KOI8R::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::KOI8R::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::KOI8R::re_r=%s; %s%s="$`$Char::KOI8R::re_r$'"; pos(%s)=length "$`$Char::KOI8R::re_r"; $Char::KOI8R::re_n++} return %s$Char::KOI8R::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::KOI8R::re_r=%s; %s%s="%s$Char::KOI8R::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::KOI8R::re_r=%s; %s%s="%s$Char::KOI8R::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::KOI8R::re_r=eval $Char::KOI8R::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/KOI8R.pm and Char/Ekoi8r.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::KOI8R;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::KOI8R;
  use Char::KOI8R ver.sion;             --- require minimum version
  use Char::KOI8R ver.sion.0;           --- expects version (match or die)

  # "no Char::KOI8R;" not supported

  or

  $ perl Char/KOI8R.pm KOI8-R_script.pl > Escaped_script.pl.e
...
Char::KOI8R software is "middleware" between perl interpreter and your Perl script
written in KOI8-R.
...
By "use Char::KOI8R;", it automatically interpret your script as KOI8-R. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/KOI8R.pm, Char/Ekoi8r.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ekoi8r.pm
works well.
...
=head1 Software Composition
   ...
   Char/KOI8R.pm               --- source code filter to escape KOI8-R
   Char/Ekoi8r.pm              --- run-time routines for Char/KOI8R.pm
...
You need write 'use Char::KOI8R;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::KOI8R;
  ---------------------------------

=head1 Calling 'Char::Ekoi8r::ignorecase()' (Char/KOI8R.pm provides)

Char/KOI8R.pm applies calling 'Char::Ekoi8r::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ekoi8r::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ekoi8r.pm provides)
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/KOI8R.pm provides)

Char/KOI8R.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/KOI8R.pm and Char/Ekoi8r.pm provide)

Insert 'Char::Ekoi8r::' at head of function name. Char/Ekoi8r.pm provides your script Char::Ekoi8r::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Ekoi8r::lc        Character
  lcfirst     Char::Ekoi8r::lcfirst   Character
  uc          Char::Ekoi8r::uc        Character
  ucfirst     Char::Ekoi8r::ucfirst   Character
  fc          Char::Ekoi8r::fc        Character
  chr         Char::Ekoi8r::chr       Character
  glob        Char::Ekoi8r::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/KOI8R.pm provides)

Char/KOI8R.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Ekoi8r.pm provides)

Char/Ekoi8r.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::KOI8R' }" at head.
Store the standard module modified for Char::KOI8R software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::KOI8R and change
'use utf8;' to 'use Char::KOI8R;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ekoi8r.pm provides these
  functions.
...
Char::KOI8R software remains one language and one interpreter.
